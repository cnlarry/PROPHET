...
/// CCI顺势指标配置
/// </summary>
public class CCIIndicatorConfig : IndicatorConfigBase
{
    ...
    public CCIIndicatorConfig()
    {
        ...
        {
            new() { IsEnabled = true, Key = "value", Label = "CCI", Color = "#FFFFFF", Thickness = 1.5 }
        };
...
public class CCIIndicator : IndicatorBase
{
    ...
    public CCIIndicator(CCIIndicatorConfig config) : base("CCI", "CCI", config) { }
    
    public override void Render(IndicatorRenderContext context)
    {
        var config = Config as CCIIndicatorConfig;
        if (config == null) return;
        
        // 如果启用多周期对比，只绘制多周期线；否则绘制主CCI线
        ...
        // 绘制参考线 (+100, 0, -100)
        var levels = new[] { 100.0, 0.0, -100.0 };
