...
IndicatorResult Calculator::MIN(
    const std::vector<double>& close,
    const IndicatorParams& params
) {
    int period = params.get_int("MIN_PERIOD", 30);
    
    if (close.size() < static_cast<size_t>(period)) {
        throw std::invalid_argument("MIN: 数据不足");
    }
    
    std::vector<double> out(close.size());
    int outBegIdx = 0, outNbElement = 0;
    
    TA_RetCode ret = TA_MIN(0, static_cast<int>(close.size()) - 1, close.data(),
                             period, &outBegIdx, &outNbElement, out.data());
    
    if (ret != TA_SUCCESS || outNbElement == 0) {
        throw std::runtime_error("MIN: TA-Lib计算失败");
    }
    ...
    // 是否创新低
    bool is_new_low = (std::abs(current_price - value) < 0.0001);
    result.set("is_new_low", Value::fromNumber(is_new_low ? 1.0 : 0.0));
