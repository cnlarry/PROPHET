...
IndicatorResult Calculator::PLUS_DM(
    const std::vector<double>& high,
    const std::vector<double>& low,
    const IndicatorParams& params
) {
    int period = params.get_int("PLUS_DM_PERIOD", 14);
    
    if (high.size() < static_cast<size_t>(period + 1)) {
        throw std::invalid_argument("PLUS_DM: 数据不足");
    }
    
    std::vector<double> out(high.size());
    int outBegIdx = 0, outNbElement = 0;
    
    TA_RetCode ret = TA_PLUS_DM(0, static_cast<int>(high.size()) - 1,
                                 high.data(), low.data(),
                                 period, &outBegIdx, &outNbElement, out.data());
    
    if (ret != TA_SUCCESS || outNbElement == 0) {
        throw std::runtime_error("PLUS_DM: TA-Lib计算失败");
    }
